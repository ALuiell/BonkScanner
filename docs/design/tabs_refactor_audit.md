...
Статус: **исторический аудит**. Он фиксирует состояние и пути на момент
проверки; выполненные исправления перечислены в
`tabs_refactor_fix_status.md`. Для текущей архитектуры используйте
`app/data_flow_architecture.md` и `docs/wiki/`. Старые имена mixin'ов и числа
тестов ниже намеренно не переписываются задним числом.
