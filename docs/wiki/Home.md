...
| [main.py](../../main.py) | Entry point of the desktop application. Instantiates `QApplication` and displays the GUI. |
| [config.py](../../config.py) | Loads, validates, and saves configuration settings, profile templates, scoring configurations, custom hotkeys, and version histories in `config.json`. |
| [updater.py](../../updater.py) | Checks for updates from releases and handles self-updates for packaged applications. |
| **User Interface (PySide6)** | |
| [gui.py](../../gui.py) | Backward compatibility facade containing core interfaces for tests and imports. |
| [gui_app.py](../../gui_app.py) | Definitive application container class (`MegabonkApp`) linking core business logic to UI events. |
| [gui_layout.py](../../gui_layout.py) | Defines the main desktop application layout, split views, and primary window panels. |
| [gui_scanner.py](../../gui_scanner.py) | Manages scanner settings UI, scan state machines, and active session control buttons. |
| [gui_run_control.py](../../gui_run_control.py) | Layout and button event mappings for resetting runs and configuring game restart options. |
| [gui_player_stats.py](../../gui_player_stats.py) | Controls the live statistics panels, item inventory styling, weapon upgrade tabs, and item sorting. |
| [gui_templates.py](../../gui_templates.py) | Dialogs and controls for creating, deleting, and tweaking template profiles. |
| [gui_dialogs.py](../../gui_dialogs.py) | Custom prompt dialogs, scoring rules adjustments, and details widgets. |
| [gui_shared.py](../../gui_shared.py) | Base classes, utility widgets, and common state-sharing interfaces for the GUI components. |
| [gui_styles.py](../../gui_styles.py) | Central styling stylesheets, color constants, tier badges, and theme parameters. |
| **Logic & Evaluators** | |
| [logic.py](../../logic.py) | Functional core that evaluates map stats against rules (Templates) and computes map scores (Scores). |
| [runtime_stats.py](../../runtime_stats.py) | Standardizes raw map details into structures suitable for matching logic. |
| [live_run_tracker.py](../../live_run_tracker.py) | Tracks live stage transitions, item acquisition differentials, and chaos stats during runs. |
| **Memory Readers & Low-level** | |
| [memory.py](../../memory.py) | Core memory access module wrapping Windows APIs (OpenProcess, ReadProcessMemory) to read raw bytes. |
| [game_data.py](../../game_data.py) | Uses pointers to read current map properties, seed, status indicators, and generation cycles. |
...
| [overlay_server.py](../../overlay_server.py) | Lightweight server hosting CSS/JS web widgets for OBS Studio overlays. |
