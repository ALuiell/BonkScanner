...
#### 1. Character Passive Bonus Tracking (Dice First Adapter)

Status: `[Archived / Implemented; 1,834-Test Regression and Dice/Chaos Stress Passed]`
...
1. Add the `Gamba` adapter behind the generic character-passive identity read.
   ...
   this adapter on the shared one-second Shrine/permanent-modifier lane;
   modifier objects persist, so a faster poll is not required to preserve
   rolls.
...
- Charge Shrines run immediately before Dice/Chaos on the same one-second
  cadence. This preserves exact ShrineLog pointer reservation while halving the
  default permanent-modifier read frequency from the former `500ms` cadence.
...
- Automated regression completed with `1,834` tests passing. Post-
  implementation live reads resolved Fox as `supported/complete`; continuous
  Dice/Chaos tracking first reached Dice `2032/2032` and Chaos `99/99`.
...
- Final one-second-cadence stress validation advanced Dice from `2032` to
  `4591` and Chaos Tome from `99` to `728`. The tracker attributed exactly
  `2,559/2,559` Dice rolls and `629/629` Chaos rolls with zero ambiguous rolls.
  A following mixed-source sample separated two additional Dice rolls (Shield
  and Crit Damage) from 11 new ShrineLog entries; two unrelated small Luck
  modifiers were correctly left outside both Dice and Chaos.
- At 5,362 permanent modifier objects, a full read plus reconstruction took
  about `0.52s` and a settled repeat read about `0.26s`, both within the
  one-second interval. The application remained responsive and produced no new
  crash log during the live run.
...
- Section 5.1 of
  ...
  now documents the verified discrete rarity/decay formula, the `n = 255`
  lower clamp, and shared pointer-based source attribution. The disproven
  continuous `random(minMultiplier, maxMultiplier)` explanation was removed.

### Chaos Tome

#### 1. Chaos Tome Fingerprint Tracking Optimization

Status: `[Archived / Implemented at 1s; Live Stress Validated]`

Original goal:

- Explore recovering Chaos Tome rolls from permanent modifier fingerprints
  during attach or full snapshots, potentially moving Chaos Tome tracking from
  the continuous `500ms` fast poll lane to the `10s` core snapshot.
...
- The `10s` cadence was not adopted. Late-attach fingerprint recovery is useful
  but can legitimately remain partial when another source creates an exact
  collision, so continuous roll-budget tracking remains the authoritative path.
- Dice and Chaos now share the Charge Shrine lane at `1s`. Permanent modifier
  objects persist, and ShrineLog pointers are reserved first on every shared
  tick, so the slower cadence changes display latency only; it does not reduce
  roll accuracy.
- Live stress validation captured `2,559` new Dice levels and `629` new Chaos
  Tome levels with exact one-for-one attribution and zero ambiguity. The
  resulting 5,362-object reconstruction remained below the one-second cadence.
...
Status: `[Archived / Requires In-Game Acceptance Test]`
