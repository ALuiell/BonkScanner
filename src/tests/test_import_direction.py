...
TOPLEVEL_DEBT: Dict[Tuple[str, str], str] = {
    ...
    ("ui/tabs/compare_runs/tab.py", "gui_shared"): (
        "Removed when gui_shared.py moves to ui/shared.py."
    ),
    ("ui/tabs/player_stats/cards.py", "gui_shared"): (
        "Removed when gui_shared.py moves to ui/shared.py."
    ),
    ("ui/tabs/player_stats/live_stats.py", "gui_shared"): (
        "Removed when gui_shared.py moves to ui/shared.py."
    ),
...
class ImportResolutionTests(unittest.TestCase):
    ...
    def test_top_level_modules_are_local(self) -> None:
        roots = local_module_roots()
        for name in ("gui_styles", "gui_shared", "live_run_tracker", "twitch_bot"):
